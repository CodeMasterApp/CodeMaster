...
class SoftwareUpdater: NSObject, ObservableObject, SPUUpdaterDelegate {
    ...
    private var appcastURL = URL(
        string: "https://github.com/CodeMasterApp/CodeMaster/releases/download/latest/appcast.xml"
    )!
    ...
    private func setFeedURL() async {
        let url = URL(string: "https://api.github.com/repos/CodeMasterApp/CodeMaster/releases/latest")!
        ...
        await MainActor.run {
            appcastURL = URL(
                string: "https://github.com/CodeMasterApp/CodeMaster/releases/download/\(result.tagName)/appcast.xml"
            )!
